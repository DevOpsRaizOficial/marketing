    # AULA 28 — Secrets em 7 min: tira a senha do código
    ...
    Senha em variável de ambiente já era. Vault é o presente.
    ...
    ## 0:25 — PONTO 1: Por que .env é só o primeiro passo (1 min)

    Quando você ouve falar em por que .env é só o primeiro passo pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: por que .env é só o primeiro passo é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: vault kv put secret/api db_pass=$(openssl rand -hex 16) | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: Vault vs Secrets Manager vs Sealed Secrets (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: vault vs secrets manager vs sealed secrets é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: External Secrets Operator K8s (1m30s)
    ...
    [COMANDO: kubectl create secret generic api-db --from-literal=pass=$VAULT_PASS | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Rotation automática (1m30s)
    ...
    [COMANDO: aws secretsmanager rotate-secret --secret-id prod/api/db | aprofundamento do conceito 4]
    ...
    1. Por que .env é só o primeiro passo
2. Vault vs Secrets Manager vs Sealed Secrets
3. External Secrets Operator K8s
4. Rotation automática
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 29. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 28/30 · Secrets em 7 min: tira a senha do código
    - **Descrição:**
        Aula 28 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-28-secrets-vault

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-28
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 Por que .env é só o primeiro passo
        01:25 Vault vs Secrets Manager vs Sealed Secrets
        02:25 External Secrets Operator K8s
        03:55 Rotation automática
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: vault kv put secret/api db_pass=$(openssl rand -hex 16)
# Comando 2: kubectl create secret generic api-db --from-literal=pass=$VAULT_PASS
# Comando 3: aws secretsmanager rotate-secret --secret-id prod/api/db
