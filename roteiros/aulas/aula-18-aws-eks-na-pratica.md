    # AULA 18 — AWS EKS em 7 min: K8s gerenciado
    ...
    EKS é o K8s da AWS. Cobra US$72/mês por cluster. Bora ver se vale.
    ...
    ## 0:25 — PONTO 1: Provisionar EKS com Terraform (1 min)

    Quando você ouve falar em provisionar eks com terraform pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: provisionar eks com terraform é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: eksctl create cluster --name devopsraiz --region us-east-1 | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: Node groups managed/Fargate (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: node groups managed/fargate é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: IRSA: IAM pra pods sem secrets (1m30s)
    ...
    [COMANDO: aws eks update-kubeconfig --name devopsraiz | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Add-ons essenciais (1m30s)
    ...
    [COMANDO: kubectl get nodes -o wide | aprofundamento do conceito 4]
    ...
    1. Provisionar EKS com Terraform
2. Node groups managed/Fargate
3. IRSA: IAM pra pods sem secrets
4. Add-ons essenciais
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 19. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 18/30 · AWS EKS em 7 min: K8s gerenciado
    - **Descrição:**
        Aula 18 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-18-aws-eks-na-pratica

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-18
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 Provisionar EKS com Terraform
        01:25 Node groups managed/Fargate
        02:25 IRSA: IAM pra pods sem secrets
        03:55 Add-ons essenciais
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: eksctl create cluster --name devopsraiz --region us-east-1
# Comando 2: aws eks update-kubeconfig --name devopsraiz
# Comando 3: kubectl get nodes -o wide
