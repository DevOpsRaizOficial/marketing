    # AULA 11 — CI/CD com GitHub Actions em 7 min
    ...
    Funciona na minha máquina é piada. CI/CD é o robô que valida pra você.
    ...
    ## 0:25 — PONTO 1: Jobs, steps, runners (1 min)

    Quando você ouve falar em jobs, steps, runners pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: jobs, steps, runners é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: cat .github/workflows/ci.yml | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: pytest no pipeline (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: pytest no pipeline é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: docker build + push GHCR (1m30s)
    ...
    [COMANDO: gh workflow run ci.yml | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Deploy automático no merge (1m30s)
    ...
    [COMANDO: gh run watch | aprofundamento do conceito 4]
    ...
    1. Jobs, steps, runners
2. pytest no pipeline
3. docker build + push GHCR
4. Deploy automático no merge
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 12. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 11/30 · CI/CD com GitHub Actions em 7 min
    - **Descrição:**
        Aula 11 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-11-ci-cd-github-actions

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-11
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 Jobs, steps, runners
        01:25 pytest no pipeline
        02:25 docker build + push GHCR
        03:55 Deploy automático no merge
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: cat .github/workflows/ci.yml
# Comando 2: gh workflow run ci.yml
# Comando 3: gh run watch
