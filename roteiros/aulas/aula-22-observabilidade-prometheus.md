    # AULA 22 — Prometheus + Grafana em 7 min: dashboards reais
    ...
    Você precisa de 3 métricas. Vou te dar todas.
    ...
    ## 0:25 — PONTO 1: Métricas RED: Rate/Errors/Duration (1 min)

    Quando você ouve falar em métricas red: rate/errors/duration pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: métricas red é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: promtool query instant http://localhost:9090 'rate(http_requests_total[5m])' | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: PromQL básico (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: promql básico é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: Alertmanager (1m30s)
    ...
    [COMANDO: promtool check rules alerts.yml | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Grafana dashboard inicial (1m30s)
    ...
    [COMANDO: curl -s localhost:9090/api/v1/query?query=up | aprofundamento do conceito 4]
...
4. Grafana dashboard inicial
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 23. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 22/30 · Prometheus + Grafana em 7 min: dashboards reais
    - **Descrição:**
        Aula 22 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-22-observabilidade-prometheus

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-22
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 Métricas RED: Rate/Errors/Duration
        01:25 PromQL básico
        02:25 Alertmanager
        03:55 Grafana dashboard inicial
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: promtool query instant http://localhost:9090 'rate(http_requests_total[5m])'
# Comando 2: promtool check rules alerts.yml
# Comando 3: curl -s localhost:9090/api/v1/query?query=up
