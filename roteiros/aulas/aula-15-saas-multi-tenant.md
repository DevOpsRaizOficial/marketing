    # AULA 15 — SaaS multi-tenant em 7 min: 1 código N clientes
    ...
    Como Salesforce e Notion isolam clientes no mesmo software.
    ...
    ## 0:25 — PONTO 1: Schema-per-tenant vs row-level (1 min)

    Quando você ouve falar em schema-per-tenant vs row-level pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: schema-per-tenant vs row-level é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: CREATE POLICY tenant_isolation ON tasks USING (tenant_id = current_setting('app.tenant')::uuid); | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: Row-level security PostgreSQL (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: row-level security postgresql é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: JWT com tenant_id (1m30s)
    ...
    [COMANDO: SET LOCAL app.tenant = '...'; | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Billing com Stripe (1m30s)
    ...
    [COMANDO: stripe customers create --email cliente@x.com | aprofundamento do conceito 4]
    ...
    1. Schema-per-tenant vs row-level
...
4. Billing com Stripe
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 16. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 15/30 · SaaS multi-tenant em 7 min: 1 código N clientes
    - **Descrição:**
        Aula 15 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-15-saas-multi-tenant

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-15
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 Schema-per-tenant vs row-level
        01:25 Row-level security PostgreSQL
        02:25 JWT com tenant_id
        03:55 Billing com Stripe
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: CREATE POLICY tenant_isolation ON tasks USING (tenant_id = current_setting('app.
# Comando 2: SET LOCAL app.tenant = '...';
# Comando 3: stripe customers create --email cliente@x.com
