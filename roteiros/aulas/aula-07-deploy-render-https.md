    # AULA 07 — Deploy no Render em 7 min: do localhost ao mundo
    ...
    Hoje eu te coloco no ar. Domínio. HTTPS. Tudo de graça.
    ...
    ## 0:25 — PONTO 1: GitHub Actions CI (1 min)

    Quando você ouve falar em github actions ci pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: github actions ci é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: git push origin main | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: Web Service no Render (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: web service no render é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: Custom domain + Let's Encrypt (1m30s)
    ...
    [COMANDO: curl https://tarefas-api.onrender.com/health | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: UptimeRobot health (1m30s)
    ...
    [COMANDO: dig api.devopsraiz.com.br | aprofundamento do conceito 4]
...
3. Custom domain + Let's Encrypt
4. UptimeRobot health
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 08. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 07/30 · Deploy no Render em 7 min: do localhost ao mundo
    - **Descrição:**
        Aula 07 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-07-deploy-render-https

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-07
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 GitHub Actions CI
        01:25 Web Service no Render
        02:25 Custom domain + Let's Encrypt
        03:55 UptimeRobot health
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: git push origin main
# Comando 2: curl https://tarefas-api.onrender.com/health
# Comando 3: dig api.devopsraiz.com.br
