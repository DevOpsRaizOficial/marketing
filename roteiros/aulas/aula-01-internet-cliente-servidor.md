    # AULA 01 — Internet em 7 minutos: cliente, servidor, DNS, HTTP
    ...
    Quando você abre um app no celular, 3 coisas acontecem em meio segundo. Vou te mostrar.
    ...
    ## 0:25 — PONTO 1: Cliente vs servidor (1 min)

    Quando você ouve falar em cliente vs servidor pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: cliente vs servidor é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: nslookup instagram.com | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: DNS: domínio vira IP (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: dns: domínio vira ip é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: HTTP métodos GET/POST/PUT/DELETE (1m30s)
    ...
    [COMANDO: curl -I https://google.com | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Códigos 2xx/4xx/5xx (1m30s)
    ...
    [COMANDO: ping 8.8.8.8 | aprofundamento do conceito 4]
...
2. DNS: domínio vira IP
3. HTTP métodos GET/POST/PUT/DELETE
4. Códigos 2xx/4xx/5xx
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 02. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 01/30 · Internet em 7 minutos: cliente, servidor, DNS, HTTP
    - **Descrição:**
        Aula 01 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-01-internet-cliente-servidor

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-01
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 Cliente vs servidor
        01:25 DNS: domínio vira IP
        02:25 HTTP métodos GET/POST/PUT/DELETE
        03:55 Códigos 2xx/4xx/5xx
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: nslookup instagram.com
# Comando 2: curl -I https://google.com
# Comando 3: ping 8.8.8.8
