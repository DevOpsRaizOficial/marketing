    # AULA 13 — OWASP Top 10 em 7 min: vulnerabilidades que matam
    ...
    Não dá pra ser sênior sem saber OWASP. Vou te dar o resumo em PT-BR.
    ...
    ## 0:25 — PONTO 1: Injection, Broken Auth, XSS (1 min)

    Quando você ouve falar em injection, broken auth, xss pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: injection, broken auth, xss é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: trivy image python:3.12-slim | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: SSRF e Insecure Design (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: ssrf e insecure design é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: Mitigações práticas (1m30s)
    ...
    [COMANDO: docker run -t owasp/zap2docker-stable zap-baseline.py -t https://exemplo.com | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Tools: ZAP, Trivy, Snyk (1m30s)
    ...
    [COMANDO: snyk test | aprofundamento do conceito 4]
    ...
    1. Injection, Broken Auth, XSS
2. SSRF e Insecure Design
3. Mitigações práticas
4. Tools: ZAP, Trivy, Snyk
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 14. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 13/30 · OWASP Top 10 em 7 min: vulnerabilidades que matam
    - **Descrição:**
        Aula 13 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-13-seguranca-owasp-top-10

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-13
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 Injection, Broken Auth, XSS
        01:25 SSRF e Insecure Design
        02:25 Mitigações práticas
        03:55 Tools: ZAP, Trivy, Snyk
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: trivy image python:3.12-slim
# Comando 2: docker run -t owasp/zap2docker-stable zap-baseline.py -t https://exemplo.com
# Comando 3: snyk test
