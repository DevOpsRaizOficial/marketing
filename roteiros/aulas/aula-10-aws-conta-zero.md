    # AULA 10 — AWS em 7 min: EC2, S3, RDS, IAM
    ...
    AWS é a líder. Vou te dar o mapa dos 4 serviços que toda vaga pede.
    ...
    ## 0:25 — PONTO 1: EC2: VMs com cost-per-hour (1 min)

    Quando você ouve falar em ec2: vms com cost-per-hour pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: ec2 é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: aws s3 ls | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: S3: storage infinito (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: s3: storage infinito é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: RDS: banco gerenciado (1m30s)
    ...
    [COMANDO: aws ec2 describe-instances --output table | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: IAM: o que derruba junior (1m30s)
    ...
    [COMANDO: aws iam list-users | aprofundamento do conceito 4]
    ...
    1. EC2: VMs com cost-per-hour
2. S3: storage infinito
3. RDS: banco gerenciado
4. IAM: o que derruba junior
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 11. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 10/30 · AWS em 7 min: EC2, S3, RDS, IAM
    - **Descrição:**
        Aula 10 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-10-aws-conta-zero

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-10
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 EC2: VMs com cost-per-hour
        01:25 S3: storage infinito
        02:25 RDS: banco gerenciado
        03:55 IAM: o que derruba junior
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: aws s3 ls
# Comando 2: aws ec2 describe-instances --output table
# Comando 3: aws iam list-users
