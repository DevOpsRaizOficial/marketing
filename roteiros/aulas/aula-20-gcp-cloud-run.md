    # AULA 20 — GCP Cloud Run em 7 min: serverless de container
    ...
    Sem servidor pra gerenciar. Paga por requisição. Escala de zero a milhares.
    ...
    ## 0:25 — PONTO 1: O que é serverless de container (1 min)

    Quando você ouve falar em o que é serverless de container pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: o que é serverless de container é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: gcloud run deploy api --source . --region us-central1 | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: gcloud run deploy (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: gcloud run deploy é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: Custom domain + IAM segundos (1m30s)
    ...
    [COMANDO: gcloud run services list | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Cloud Run Jobs batch (1m30s)
    ...
    [COMANDO: gcloud run jobs execute migracao-db --wait | aprofundamento do conceito 4]
    ...
    1. O que é serverless de container
2. gcloud run deploy
3. Custom domain + IAM segundos
4. Cloud Run Jobs batch
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 21. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 20/30 · GCP Cloud Run em 7 min: serverless de container
    - **Descrição:**
        Aula 20 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-20-gcp-cloud-run

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-20
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 O que é serverless de container
        01:25 gcloud run deploy
        02:25 Custom domain + IAM segundos
        03:55 Cloud Run Jobs batch
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: gcloud run deploy api --source . --region us-central1
# Comando 2: gcloud run services list
# Comando 3: gcloud run jobs execute migracao-db --wait
