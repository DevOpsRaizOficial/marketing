    # AULA 12 — Observabilidade em 7 min: logs, métricas, traces
    ...
    Você só sabe que o sistema tá morto quando o cliente reclama? Vamos resolver.
    ...
    ## 0:25 — PONTO 1: 3 pilares: logs/métricas/traces (1 min)

    Quando você ouve falar em 3 pilares: logs/métricas/traces pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: 3 pilares é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: docker run -p 9090:9090 prom/prometheus | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: Logs estruturados JSON (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: logs estruturados json é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: Prometheus + Grafana (1m30s)
    ...
    [COMANDO: curl http://localhost:9090/metrics | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: OpenTelemetry (1m30s)
    ...
    [COMANDO: kubectl logs -l app=api --tail=50 -f | aprofundamento do conceito 4]
    ...
    1. 3 pilares: logs/métricas/traces
...
4. OpenTelemetry
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 13. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 12/30 · Observabilidade em 7 min: logs, métricas, traces
    - **Descrição:**
        Aula 12 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-12-logs-e-monitoramento

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-12
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 3 pilares: logs/métricas/traces
        01:25 Logs estruturados JSON
        02:25 Prometheus + Grafana
        03:55 OpenTelemetry
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: docker run -p 9090:9090 prom/prometheus
# Comando 2: curl http://localhost:9090/metrics
# Comando 3: kubectl logs -l app=api --tail=50 -f
