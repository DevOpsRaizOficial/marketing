    # AULA 17 — Helm e Kustomize em 7 min: templating K8s
    ...
    YAML manual vira insustentável depois do 3º microsserviço.
    ...
    ## 0:25 — PONTO 1: Helm Chart: values, templates (1 min)

    Quando você ouve falar em helm chart: values, templates pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: helm chart é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: helm install api ./chart --values prod.yaml | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: Kustomize: bases + overlays (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: kustomize: bases + overlays é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: Quando usar cada um (1m30s)
    ...
    [COMANDO: kustomize build overlays/prod | kubectl apply -f - | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Argo CD GitOps (1m30s)
    ...
    [COMANDO: argocd app sync devopsraiz | aprofundamento do conceito 4]
    ...
    1. Helm Chart: values, templates
2. Kustomize: bases + overlays
...
4. Argo CD GitOps
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 18. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 17/30 · Helm e Kustomize em 7 min: templating K8s
    - **Descrição:**
        Aula 17 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-17-k8s-helm-kustomize

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-17
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 Helm Chart: values, templates
        01:25 Kustomize: bases + overlays
        02:25 Quando usar cada um
        03:55 Argo CD GitOps
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: helm install api ./chart --values prod.yaml
# Comando 2: kustomize build overlays/prod | kubectl apply -f -
# Comando 3: argocd app sync devopsraiz
