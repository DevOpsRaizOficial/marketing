    # AULA 23 — SLO e SLI em 7 min: o jogo dos 9s
    ...
    99,9% de uptime é 8h46min de downtime/ano. 99,99% é 52 min. SRE é matemática.
    ...
    ## 0:25 — PONTO 1: SLI, SLO, SLA na prática (1 min)

    Quando você ouve falar em sli, slo, sla na prática pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: sli, slo, sla na prática é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: echo 'SLO 99.9% = 43min downtime/mes' | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: Error budget: moeda do SRE (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: error budget: moeda do sre é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: Burn rate alerts (1m30s)
    ...
    [COMANDO: curl -s api/health | jq .uptime | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Postmortem culpa-zero (1m30s)
    ...
    [COMANDO: kubectl get pdb -A | aprofundamento do conceito 4]
    ...
    1. SLI, SLO, SLA na prática
2. Error budget: moeda do SRE
3. Burn rate alerts
4. Postmortem culpa-zero
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 24. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 23/30 · SLO e SLI em 7 min: o jogo dos 9s
    - **Descrição:**
        Aula 23 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-23-sre-slo-slis

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-23
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 SLI, SLO, SLA na prática
        01:25 Error budget: moeda do SRE
        02:25 Burn rate alerts
        03:55 Postmortem culpa-zero
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: echo 'SLO 99.9% = 43min downtime/mes'
# Comando 2: curl -s api/health | jq .uptime
# Comando 3: kubectl get pdb -A
