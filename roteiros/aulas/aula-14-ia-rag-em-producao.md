    # AULA 14 — RAG em 7 min: IA em cima dos seus dados
    ...
    ChatGPT em cima dos seus docs. Custa centavos por consulta. Bora montar.
    ...
    ## 0:25 — PONTO 1: O que é RAG (retrieval + generation) (1 min)

    Quando você ouve falar em o que é rag (retrieval + generation) pela primeira vez, parece complicado, mas tem uma analogia simples. Pensa numa entrega de pacote: cliente é quem pede, servidor é quem entrega. É só isso. Resumindo: o que é rag (retrieval + generation) é a base de toda comunicação que vai aparecer nas próximas aulas.

    [COMANDO: pip install langchain openai pgvector | rodando no terminal pra testar]

    ---

    ## 1:25 — PONTO 2: Vector DB: Pinecone/pgvector (1 min)

    Esse é o ponto que mais derruba dev júnior em entrevista. Anota: vector db: pinecone/pgvector é o mecanismo por trás de tudo que funciona em rede. Sem isso, você não consegue chegar em nenhum site, app, ou API. Olha o comando que prova isso na sua máquina agora.

    ---

    ## 2:25 — PONTO 3: Embeddings + similarity search (1m30s)
    ...
    [COMANDO: python ingest_docs.py --dir ./docs/ | exemplo prático do conceito 3]

    ---

    ## 3:55 — PONTO 4: Custo por 1k consultas (1m30s)
    ...
    [COMANDO: curl -X POST localhost:8000/chat -d '{"q":"como deploy?"}' | aprofundamento do conceito 4]
    ...
    1. O que é RAG (retrieval + generation)
2. Vector DB: Pinecone/pgvector
3. Embeddings + similarity search
4. Custo por 1k consultas
    ...
    Tira-dúvidas direto comigo no WhatsApp **(11) 96482-3126**. Te vejo na aula 15. Valeu raiz.

    ---

    ## METADADOS YOUTUBE

    - **Título:** Aula 14/30 · RAG em 7 min: IA em cima dos seus dados
    - **Descrição:**
        Aula 14 da série "Zero ao Deploy" do canal DEVOPSRAIZ.

        📌 RECURSOS DESSA AULA:

        🆓 Comandos e snippets (gratuito, GitHub público):
        → https://github.com/DevOpsRaizOficial/aulas-publicas/tree/main/aula-14-ia-rag-em-producao

        💎 PDF de Apoio Completo (R$ 9,90 → R$ 4,95 com CUPOM50):
        → Resumo + comandos + troubleshooting + exercícios + checklist
        → https://go.hotmart.com/PDF-AULA-14
        ...
        00:00 Hook
        00:15 Apresentação
        00:25 O que é RAG (retrieval + generation)
        01:25 Vector DB: Pinecone/pgvector
        02:25 Embeddings + similarity search
        03:55 Custo por 1k consultas
        05:25 Recap
        05:55 CTAs + cupons

        --- COMANDOS USADOS ---
        # Comando 1: pip install langchain openai pgvector
# Comando 2: python ingest_docs.py --dir ./docs/
# Comando 3: curl -X POST localhost:8000/chat -d '{"q":"como deploy?"}'
